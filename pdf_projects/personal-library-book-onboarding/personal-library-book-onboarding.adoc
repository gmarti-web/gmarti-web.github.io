...
If one way fails, try the next one. See the following flowchart for reference.

.New book process flowchart
image::resources/images/add-new-book.drawio.png[A flowchart showing decisions a user must make when adding a book to a Libib collection., height=500]
...
If you can't find the book by its barcode, ISBN 13 or 10, or title, create a new book entry.
